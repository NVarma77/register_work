...
class UnsignedAutoEncoderTopK(Dictionary, nn.Module):
    """
    The top-k autoencoder architecture and initialization used in https://arxiv.org/abs/2406.04093
    NOTE: (From Adam Karvonen) There is an unmaintained implementation using Triton kernels in the topk-triton-implementation branch.
    We abandoned it as we didn't notice a significant speedup and it added complications, which are noted
    ...
    def encode(self, x: t.Tensor, return_topk: bool = False, use_threshold: bool = False):
        #post_relu_feat_acts_BF = nn.functional.relu(self.encoder(x - self.b_dec))
        pre_abs_feat_acts_BF = self.encoder(x - self.b_dec)
        post_abs_feat_acts_BF = pre_abs_feat_acts_BF.abs()

        if use_threshold:
            encoded_acts_BF = post_abs_feat_acts_BF * (post_abs_feat_acts_BF > self.threshold)
            if return_topk:
                post_topk = post_abs_feat_acts_BF.topk(self.k, sorted=False, dim=-1)
                return encoded_acts_BF, post_topk.values, post_topk.indices, post_abs_feat_acts_BF
            else:
                return encoded_acts_BF
        ...
        #tops_acts_BK = post_topk.values
        top_indices_BK = post_topk.indices
        row_indices = t.arange(top_indices_BK.size(0)).unsqueeze(1).expand_as(top_indices_BK)

        tops_acts_BK = pre_abs_feat_acts_BF[row_indices, top_indices_BK]

        buffer_BF = t.zeros_like(post_abs_feat_acts_BF)
        encoded_acts_BF = buffer_BF.scatter_(dim=-1, index=top_indices_BK, src=tops_acts_BK)
        #print('encoded_acts_BF', encoded_acts_BF.sum(dim=-1))

        if return_topk:
            return encoded_acts_BF, tops_acts_BK, top_indices_BK, post_abs_feat_acts_BF
        else:
            return encoded_acts_BF
    ...
    def from_pretrained(path, k: Optional[int] = None, device=None):
        ...
        autoencoder = UnsignedAutoEncoderTopK(activation_dim, dict_size, k)
        autoencoder.load_state_dict(state_dict)
        if device is not None:
            autoencoder.to(device)
        return autoencoder
